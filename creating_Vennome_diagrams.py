...
def get_filename():
    ...
    root = tk.Tk()
    root.withdraw()
    filename = simpledialog.askstring("Введите название файла", "Пожалуйста, введите название файла:", parent=root)
    root.destroy()
    if filename:
        messagebox.showinfo("Название файла", f'Файл {filename} успешно создан в папке data_PDF')
        return filename
    else:
        messagebox.showwarning("Ошибка", "Вы не ввели название файла.")
        return None


filename = get_filename()

with PdfPages(f'data_PDF/{filename}.pdf') as pdf:
