...
for i in range(400):
    ...
    vaccinations = random.choice([True, False])
    pets = random.choice([True, False])

    # Добавление строки в список
    data.append([name, birth_year, hobby, class_year, city, paid_study, free_study, driver_license, smoking, vaccinations, pets])

# Создание DataFrame из списка
df = pd.DataFrame(data, columns=["Имя", "Год рождения", "Хобби", "Класс обучения", "Место жительства", "Платное обучение", "Бесплатное обучение", "Водительские права", "Курение", "Прививки", "Домашние животные"])
